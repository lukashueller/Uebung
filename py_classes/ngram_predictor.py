...
class NGRAM_Predictor:
    ...
    def __init__(self, chat_data):
        docs = [
            message.message for message in chat_data if message.message_type == MessageTypes.TEXT]
        ...
        counts = self.vectorizer.fit_transform(
            docs)  # counts = document term matrix

        self.features = self.vectorizer.get_feature_names_out()
        self.sum_vec = counts.sum(axis=0).tolist()[0]
        self.feature_matrix = [(f, self.sum_vec[i]) for i, f in enumerate(
            self.features)]  # This is already sorted
    ...
    def get_predictions(self, can_complete_word, input_array):
        ...
        for ngram_length in range(len(preprocessed_input), 0, -1):
            ...
            result = [
                ...
                for res
                in result
                if res[1][1:].count(" ") < 1 and res[1] != sss and res[1] != ""
            ]
